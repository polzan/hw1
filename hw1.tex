...
converge very quickly but the value of the MSE at convergence
$J(\infty)$ is more than \SI{3}{\dB} over $J_{min}$ (and $J_{ex} =
6.17\cdot10^{-4}$). A smaller value, like $\tilde{\mu} = 0.25$,
...
obtainable by a linear predictor, the excess MSE $J_{ex} =
8.82\cdot10^{-5}$ is almost ten times lower than before and the value
of the MSE at convergence remains \SI{0.58}{\dB} over the optimal
value.
%% in accordance with
